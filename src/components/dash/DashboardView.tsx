...
interface DashboardViewProps {
  lang: Language;
  currentUser: User;
  accessToken?: string; // Absent tant que la session n'est pas active : getDashboardData()
  // renvoie alors null et l'affichage garde ses valeurs mock/props existantes (voir plus bas).
  domains: Domain[];
...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  // Consentement légal obligatoire avant tout scan externe (même exigence que HomeView/ScanView).
  const [legalAccepted, setLegalAccepted] = useState(false);
  // Vrai appel réseau en cours vers scanController (Nuclei + Nmap + SSL Labs), pas une simulation.
  const [isAddingDomain, setIsAddingDomain] = useState(false);
  const [addError, setAddError] = useState('');

  // Une fois qu'on dispose de vraies données (chargées via getDashboardData ou ajoutées via
  // startScan), on ne laisse plus les props mock du parent (App.tsx) écraser silencieusement
  // l'état réel de ce composant.
  ...
  useEffect(() => {
    ...
    GhulabeBackend.getDashboardData(accessToken).then((result) => {
      if (!result) return; // Pas de résultat (pas de token, erreur réseau) : on garde mock/props.
      if (result.domains.length > 0) {
        setRealDomains(result.domains as Domain[]);
        setHasRealData(true);
      }
  ...
  const closeAddModal = () => {
    if (isAddingDomain) return; // Ne pas fermer pendant un scan en cours.
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    if (!newDomainUrl || !newDomainUrl.includes('.')) {
      setAddError(lang === 'fr' 
        ? "Veuillez entrer une URL de domaine valide (ex: masociete-douala.cm)" 
        : "Please enter a valid domain URL (e.g., mybusiness.com)"
      );
      return;
    }
    ...
    if (!legalAccepted) {
      setAddError(lang === 'fr' 
        ? "Vous devez accepter les conditions d'audit externe avant de lancer le scan." 
        : "You must accept the external audit terms before starting the scan."
      );
      return;
    }
    ...
    try {
      const result = await GhulabeBackend.startScan(newDomainUrl, legalAccepted, accessToken);

      const score = typeof result.score === 'number' ? result.score : 7.0;
      const newDom: Domain = {
        id: result.domain_id || result.id || `dom-${Date.now()}`,
        user_id: currentUser.id,
        url: newDomainUrl,
        last_scan: lang === 'fr' ? "À l'instant" : "Just now",
        score,
        status: score >= 8 ? 'safe' : score >= 5 ? 'warning' : 'critical'
      };

      setRealDomains((prev) => [newDom, ...prev]);
      setRealScansCount((prev) => prev + 1);
      setHasRealData(true);

      // Notifie le parent (App.tsx) pour compat/cohérence de son propre état, sans que ça
      // n'écrase plus notre affichage réel (voir garde hasRealData ci-dessus).
      onAddDomain(newDomainUrl);

      setShowAddModal(false);
      resetAddModalState();
    } catch (err: any) {
      setAddError(err?.message || (lang === 'fr' 
        ? "Erreur lors du scan du domaine. Veuillez réessayer." 
        : "Error scanning domain. Please try again."
      ));
    } finally {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-6 space-y-4 sm:space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        {/* Right Col: Alerts (SMS / Email Simulation) */}
          ...
          <div className="glass-card p-8 rounded-3xl max-w-md w-full border border-[#0066FF] shadow-[0_0_40px_rgba(0,102,255,0.3)] space-y-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-gray-400 hover:text-white font-mono text-sm disabled:opacity-30 disabled:cursor-not-allowed"
              >
                ✕
              </button>
            </div>
            ...
            <form onSubmit={handleAddSubmit} className="space-y-4">
              <input
                ...
                className="w-full p-3.5 rounded-xl bg-[#0A0A0F] border border-[#0066FF]/50 text-white font-mono text-sm focus:border-[#00FF88] focus:outline-none disabled:opacity-50"
              />

              {/* Consentement légal obligatoire (même exigence que HomeView/ScanView) */}
              ...
              {addError && (
                <p className="text-xs font-semibold text-[#FF2D2D] flex items-center gap-1.5">
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button
                  ...
                  onClick={closeAddModal}
                  disabled={isAddingDomain}
                  className="px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-gray-300 text-xs font-mono cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed"
                >
                  Annuler
                </button>
                <button
                  type="submit"
                  disabled={isAddingDomain}
                  className="px-5 py-2.5 rounded-xl bg-[#0066FF] hover:bg-[#00FF88] text-white hover:text-[#0A0A0F] font-display font-bold text-xs uppercase transition-all cursor-pointer shadow-[0_0_15px_rgba(0,102,255,0.5)] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isAddingDomain ? (
                    <>
